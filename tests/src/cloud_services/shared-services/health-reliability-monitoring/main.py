"""
Test shim that loads the real health_reliability_monitoring main module from src/.
"""
...
def _load_real_main():
    root = Path(__file__).resolve().parents[5] / "src" / "cloud_services" / "shared-services" / "health-reliability-monitoring"
    ...
    routes_path = root / "routes" / "__init__.py"
    spec_routes = importlib.util.spec_from_file_location(f"{pkg_name}.routes", routes_path, submodule_search_locations=[str(root / "routes")])
