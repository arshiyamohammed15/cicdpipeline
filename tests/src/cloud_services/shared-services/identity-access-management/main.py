"""
Test shim that loads the real IAM main module from src/.
"""
...
def _load_real_main():
    root = Path(__file__).resolve().parents[5] / "src" / "cloud_services" / "shared-services" / "identity-access-management"
    real_main = root / "main.py"
    if not real_main.exists():
        raise FileNotFoundError(f"Real main module not found at {real_main}")

    pkg_name = "identity_access_management"
