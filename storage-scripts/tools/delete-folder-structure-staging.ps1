<#
.SYNOPSIS
  ZeroUI Folder Structure Deletion Script - Staging Environment

.DESCRIPTION
  Deletes the folder structure for staging environment created by create-folder-structure-staging.ps1.
  Deletes tenant, product, and shared planes under {ZU_ROOT}/staging.
...
.EXAMPLE
  ...
  .\delete-folder-structure-staging.ps1 -Drive "D" -ProductName "ZeroUI"
  .\delete-folder-structure-staging.ps1 -ZuRoot "D:\ZeroUI"
  .\delete-folder-structure-staging.ps1 -ZuRoot "D:\ZeroUI" -Force
  .\delete-folder-structure-staging.ps1 -ZuRoot "D:\ZeroUI" -WhatIf

.NOTES
  Version: 2.0
  Based on: folder-business-rules.md
  WARNING: This script will delete all folders under the staging environment.
  It will NOT delete the ZU_ROOT directory itself, only the staging environment folder.
#>
...
Write-Host "`nDeleting ZeroUI folder structure for staging environment under: $ZuRoot" -ForegroundColor Cyan
Write-Host "==========================================`n" -ForegroundColor Cyan

if($WhatIf) {
    Write-Host "WHATIF MODE: No actual deletions will be performed.`n" -ForegroundColor Yellow
} elseif(-not $Force) {
    Write-Host "WARNING: This will delete all folders under the staging environment." -ForegroundColor Red
...
# ============================================================================
$envPath = Join-Path $ZuRoot "staging"
if(-not (Test-Path $envPath)) {
    Write-Host "Staging environment folder does not exist: $envPath" -ForegroundColor Yellow
    exit 0
}

Write-Host "Deleting staging environment folders..." -ForegroundColor Yellow
...
$errors += if(-not (Remove-FolderStructure -Path $envPath -Description "staging environment")) { "staging" } else { $null; $deletedCount++ }
