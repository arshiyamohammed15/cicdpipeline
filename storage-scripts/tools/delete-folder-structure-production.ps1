<#
.SYNOPSIS
  ZeroUI Folder Structure Deletion Script - Production Environment

.DESCRIPTION
  Deletes the folder structure for production environment created by create-folder-structure-production.ps1.
  Deletes tenant, product, and shared planes under {ZU_ROOT}/production.
...
.EXAMPLE
  ...
  .\delete-folder-structure-production.ps1 -Drive "D" -ProductName "ZeroUI"
  .\delete-folder-structure-production.ps1 -ZuRoot "D:\ZeroUI"
  .\delete-folder-structure-production.ps1 -ZuRoot "D:\ZeroUI" -Force
  .\delete-folder-structure-production.ps1 -ZuRoot "D:\ZeroUI" -WhatIf

.NOTES
  Version: 2.0
  Based on: folder-business-rules.md
  WARNING: This script will delete all folders under the production environment.
  It will NOT delete the ZU_ROOT directory itself, only the production environment folder.
#>
...
Write-Host "`nDeleting ZeroUI folder structure for production environment under: $ZuRoot" -ForegroundColor Cyan
Write-Host "==========================================`n" -ForegroundColor Cyan

if($WhatIf) {
    Write-Host "WHATIF MODE: No actual deletions will be performed.`n" -ForegroundColor Yellow
} elseif(-not $Force) {
    Write-Host "WARNING: This will delete all folders under the production environment." -ForegroundColor Red
...
if(-not (Test-Path $envPath)) {
    Write-Host "Production environment folder does not exist: $envPath" -ForegroundColor Yellow
    exit 0
}

Write-Host "Deleting production environment folders..." -ForegroundColor Yellow
...
$errors += if(-not (Remove-FolderStructure -Path $envPath -Description "production environment")) { "production" } else { $null; $deletedCount++ }
