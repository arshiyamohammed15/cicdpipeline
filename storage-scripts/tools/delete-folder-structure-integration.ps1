<#
.SYNOPSIS
  ZeroUI Folder Structure Deletion Script - Integration Environment

.DESCRIPTION
  Deletes the folder structure for integration environment created by create-folder-structure-integration.ps1.
  Deletes tenant, product, and shared planes under {ZU_ROOT}/integration.
...
.EXAMPLE
  ...
  .\delete-folder-structure-integration.ps1 -Drive "D" -ProductName "ZeroUI"
  .\delete-folder-structure-integration.ps1 -ZuRoot "D:\ZeroUI"
  .\delete-folder-structure-integration.ps1 -ZuRoot "D:\ZeroUI" -Force
  .\delete-folder-structure-integration.ps1 -ZuRoot "D:\ZeroUI" -WhatIf

.NOTES
  Version: 2.0
  Based on: folder-business-rules.md
  WARNING: This script will delete all folders under the integration environment.
  It will NOT delete the ZU_ROOT directory itself, only the integration environment folder.
#>
...
Write-Host "`nDeleting ZeroUI folder structure for integration environment under: $ZuRoot" -ForegroundColor Cyan
Write-Host "==========================================`n" -ForegroundColor Cyan

if($WhatIf) {
    Write-Host "WHATIF MODE: No actual deletions will be performed.`n" -ForegroundColor Yellow
} elseif(-not $Force) {
    Write-Host "WARNING: This will delete all folders under the integration environment." -ForegroundColor Red
...
# ============================================================================
# Delete integration environment
# ============================================================================
$envPath = Join-Path $ZuRoot "integration"
if(-not (Test-Path $envPath)) {
    Write-Host "Integration environment folder does not exist: $envPath" -ForegroundColor Yellow
    exit 0
}

Write-Host "Deleting integration environment folders..." -ForegroundColor Yellow
...
$errors += if(-not (Remove-FolderStructure -Path $envPath -Description "integration environment")) { "integration" } else { $null; $deletedCount++ }
...
if($errorCount -eq 0) {
    if($WhatIf) {
        ...
        Write-Host "Environment: integration" -ForegroundColor Cyan
