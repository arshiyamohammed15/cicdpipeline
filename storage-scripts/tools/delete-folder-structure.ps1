<#
.SYNOPSIS
  ZeroUI Folder Structure Deletion Script

.DESCRIPTION
  Deletes the folder structure created by create-folder-structure.ps1.
...
.EXAMPLE
  ...
  .\delete-folder-structure.ps1 -Drive "D" -ProductName "ZeroUI"
  .\delete-folder-structure.ps1 -Drive "E" -ProductName "MyProduct"
  .\delete-folder-structure.ps1 -ZuRoot "D:\ZeroUI"
  .\delete-folder-structure.ps1 -ZuRoot "D:\ZeroUI" -Force
  .\delete-folder-structure.ps1 -ZuRoot "D:\ZeroUI" -WhatIf

.NOTES
  Version: 2.0
  Based on: folder-business-rules.md
  WARNING: This script will delete all folders under the environments.
  It will NOT delete the ZU_ROOT directory itself, only its contents related to the folder structure.
#>
...
Write-Host "`nDeleting ZeroUI folder structure under: $ZuRoot" -ForegroundColor Cyan
Write-Host "==========================================`n" -ForegroundColor Cyan

if($WhatIf) {
    Write-Host "WHATIF MODE: No actual deletions will be performed.`n" -ForegroundColor Yellow
} elseif(-not $Force) {
    Write-Host "WARNING: This will delete all folders under the environments (development, integration, staging, production)." -ForegroundColor Red
...
# ============================================================================
# Delete four environments
# ============================================================================
$environments = @("development", "integration", "staging", "production")

foreach($env in $environments) {
    Write-Host "`nDeleting $env environment folders..." -ForegroundColor Yellow

    $envPath = Join-Path $ZuRoot $env
    if(-not (Test-Path $envPath)) {
        Write-Verbose "Environment folder does not exist: $envPath"
        continue
    }
    ...
    $errors += if(-not (Remove-FolderStructure -Path $envPath -Description "$env environment")) { $env } else { $null; $deletedCount++ }
}
