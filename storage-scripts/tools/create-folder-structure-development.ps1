<#
.SYNOPSIS
  ZeroUI Folder Structure Creator - Development Environment

.DESCRIPTION
  Creates the folder structure for development environment as per folder-business-rules.md v2.0.
  Creates IDE plane, tenant, product, and shared planes under {ZU_ROOT}/development.
...
.EXAMPLE
  ...
  .\create-folder-structure-development.ps1 -Drive "D" -ProductName "ZeroUI"
  .\create-folder-structure-development.ps1 -ZuRoot "D:\ZeroUI"
  .\create-folder-structure-development.ps1 -ZuRoot "D:\ZeroUI" -CompatAliases
...
# Function to create IDE Plane
function New-IDEFolderStructure {
    param(
        [string]$BasePath,
        [ref]$Errors
    )
...
Write-Host "`nCreating ZeroUI folder structure for development environment under: $ZuRoot" -ForegroundColor Cyan
Write-Host "==========================================`n" -ForegroundColor Cyan

$errors = @()

# ============================================================================
# Create development environment with IDE, tenant, product, and shared planes
# ============================================================================
$envPath = Join-Path $ZuRoot "development"
$errors += if(-not (New-FolderStructure -Path $envPath -Description "development environment")) { "development" }

# IDE plane only under development
Write-Host "Creating IDE Plane for development..." -ForegroundColor Cyan
New-IDEFolderStructure -BasePath $envPath -Errors ([ref]$errors)

# Tenant, Product, and Shared planes
Write-Host "Creating Tenant, Product, and Shared planes for development..." -ForegroundColor Cyan
...
if($errorCount -eq 0) {
    ...
    Write-Host "IDE Plane: $ZuRoot\development\ide" -ForegroundColor Cyan
    Write-Host "Planes: IDE, tenant, product, shared" -ForegroundColor Cyan
    if($CompatAliases) {
