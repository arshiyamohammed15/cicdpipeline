# ZEROUI 2.0 CONSTITUTION - 71 UNIQUE RULES
...
Instead of writing "use 10 apples" in your recipe, write "use [number] apples" and keep the number 10 in a separate note. This way you can easily change to 12 apples later. Prefer adding new features rather than changing old ones. Use on/off switches for features. Write down how to go back if something doesn't work.
...
Programs should start faster than microwaving popcorn (under 2 seconds). Buttons should respond instantly when clicked (under 0.1 seconds). Don't use too much computer memory - like not hogging all the closet space.
