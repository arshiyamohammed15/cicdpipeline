<#
.SYNOPSIS
  Shared Cloud Local Bootstrap - Start Shared Plane services locally

.DESCRIPTION
  Bootstraps and manages the Shared Cloud plane stack (shared services + dependencies)
  on a local laptop for running shared-plane capabilities (PKI/trust, registry, SIEM hooks, notifications, evaluation harness deps).
...
.PARAMETER StartDockerDeps
  If set, starts Shared Postgres + Redis via docker compose
...
.PARAMETER RunUnitTests
  If set, runs shared-plane unit tests; FAIL if any fail
...
.EXAMPLE
  .\shared_cloud_bootstrap_local.ps1 -Mode setup -StartDockerDeps

.EXAMPLE
  .\shared_cloud_bootstrap_local.ps1 -Mode start
#>
...
# Discover shared services
function Discover-SharedServices {
    param([string]$RepoRoot)
    
    $services = @{}
    
    # Core shared services (infrastructure)
    $sharedServices = @(
        @{Name="key-management-service"; Path="src\cloud_services\shared-services\key-management-service"; Port=8020; HealthPath="/health"},
        @{Name="contracts-schema-registry"; Path="src\cloud_services\shared-services\contracts-schema-registry"; Port=8021; HealthPath="/health"},
        @{Name="identity-access-management"; Path="src\cloud_services\shared-services\identity-access-management"; Port=8022; HealthPath="/health"},
        @{Name="alerting-notification-service"; Path="src\cloud_services\shared-services\alerting-notification-service"; Port=8023; HealthPath="/healthz"},
        @{Name="budgeting-rate-limiting-cost-observability"; Path="src\cloud_services\shared-services\budgeting-rate-limiting-cost-observability"; Port=8024; HealthPath="/health"},
        @{Name="configuration-policy-management"; Path="src\cloud_services\shared-services\configuration-policy-management"; Port=8025; HealthPath="/health"},
        @{Name="data-governance-privacy"; Path="src\cloud_services\shared-services\data-governance-privacy"; Port=8026; HealthPath="/health"},
        @{Name="evidence-receipt-indexing-service"; Path="src\cloud_services\shared-services\evidence-receipt-indexing-service"; Port=8027; HealthPath="/health"},
        @{Name="health-reliability-monitoring"; Path="src\cloud_services\shared-services\health-reliability-monitoring"; Port=8028; HealthPath="/health"},
        @{Name="ollama-ai-agent"; Path="src\cloud_services\shared-services\ollama-ai-agent"; Port=8029; HealthPath="/health"},
        @{Name="trust-as-capability"; Path="src\cloud_services\shared-services\trust-as-capability"; Port=8030; HealthPath="/health"}
    )
...
Write-Host "Shared Cloud Local Bootstrap" -ForegroundColor Cyan
...
# Discover services
$DiscoveredServices = Discover-SharedServices -RepoRoot $RepoRoot
Write-Info "Discovered $($DiscoveredServices.Count) shared service(s)"
...
if ($Mode -eq "setup") {
    ...
    # Update .env with shared DB URLs
    $dbVars = @{
    ...
    if (Test-Path $folderScript) {
        ...
        # Create minimal shared folders
        Write-Info "Creating minimal shared folder structure..."
        $sharedBase = Join-Path $ZuRootResolved "shared"
        $sharedDirs = @(
            (Join-Path $sharedBase "pki"),
            (Join-Path $sharedBase "registry"),
            (Join-Path $sharedBase "telemetry"),
            (Join-Path $sharedBase "siem"),
            (Join-Path $sharedBase "governance"),
            (Join-Path $sharedBase "notifications"),
            (Join-Path $sharedBase "evaluation"),
            (Join-Path $sharedBase "bi-lake"),
            (Join-Path $sharedBase "queue")
        )
...
if ($StartDockerDeps) {
    ...
    $composeFile = Join-Path $RepoRoot "infra\local\docker-compose.shared_cloud.local.yml"
    $envFile = Join-Path $RepoRoot "infra\local\shared_cloud.local.env"
    ...
    if ($Mode -eq "start" -or $Mode -eq "setup") {
        Write-Info "Starting Shared Postgres and Redis containers..."
        try {
        ...
        while ($waited -lt $maxWait) {
            ...
            $redis = & docker ps --filter "name=zeroui-redis-shared" --format "{{.Status}}" 2>&1
            
            if ($sharedPg -match "healthy" -and $redis -match "healthy") {
                $allHealthy = $true
                break
            }
...
$logDir = Join-Path $env:USERPROFILE ".zeroai\shared\logs"
if (-not (Test-Path $logDir)) {
    New-Item -ItemType Directory -Path $logDir -Force | Out-Null
}

if ($Mode -eq "start" -or ($Mode -eq "setup" -and $StartDockerDeps)) {
    Write-Host "`n--- Starting Shared Cloud Services ---" -ForegroundColor Yellow
    ...
    # Start services in dependency order (infrastructure first)
    $startOrder = @("key-management-service", "identity-access-management", "contracts-schema-registry",
                    "configuration-policy-management", "budgeting-rate-limiting-cost-observability",
                    "data-governance-privacy", "evidence-receipt-indexing-service",
                    "alerting-notification-service", "health-reliability-monitoring",
                    "ollama-ai-agent", "trust-as-capability")
    ...
    Write-Host "`n--- Stopping Shared Cloud Services ---" -ForegroundColor Yellow
    ...
    if (Test-Path $pidFile) {
        ...
        foreach ($processId in $pids) {
            try {
                ...
                Write-Warning-Custom "Could not stop process $processId : $($_.Exception.Message)"
            }
        }
    ...
    foreach ($svcName in $DiscoveredServices.Keys) {
        ...
        if ($conn) {
            ...
            try {
                Stop-Process -Id $processId -Force
                Write-Success "Stopped service on port $($svc.Port) (PID: $processId)"
            } catch {
    ...
    & docker ps --filter "name=zeroui-shared" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
    
    Write-Info "`nShared services:"
    foreach ($svcName in $DiscoveredServices.Keys) {
        $svc = $DiscoveredServices[$svcName]
        $conn = Get-NetTCPConnection -LocalPort $svc.Port -ErrorAction SilentlyContinue
        if ($conn) {
...
if ($RunUnitTests) {
    Write-Host "`n--- Running Shared Unit Tests ---" -ForegroundColor Yellow
    
    $venvPath = Join-Path $RepoRoot "venv"
    $pythonExe = Join-Path $venvPath "Scripts\python.exe"
    
    if (Test-Path $pythonExe) {
        Write-Info "Running shared-plane unit tests..."
        try {
            Push-Location $RepoRoot
            # Run tests for shared services
            $testPath = "tests\cloud_services\shared_services"
            if (Test-Path $testPath) {
                & $pythonExe -m pytest -q $testPath -v
            } else {
                Write-Warning-Custom "Shared test directory not found. Skipping tests."
                Pop-Location
                exit 0
            }
            if ($LASTEXITCODE -ne 0) {
                Write-Error-Custom "Shared unit tests failed (exit code: $LASTEXITCODE)"
                Pop-Location
                exit 1
            }
            Write-Success "ALL SHARED TESTS PASSED"
            Pop-Location
        } catch {
            Pop-Location
            Write-Error-Custom "Shared unit tests failed: $_"
            exit 1
        }
    } else {
        Write-Error-Custom "Python venv not found. Run with -Mode setup first."
...
if ($StartDockerDeps) {
    ...
    & docker ps --filter "name=zeroui-shared" --format "  {{.Names}}: {{.Status}}" 2>&1 | ForEach-Object { Write-Host $_ -ForegroundColor White }
    
    Write-Host "`nDatabase URLs:" -ForegroundColor Cyan
    Write-Host "  Shared: postgresql://zeroui:zeroui_dev_only@localhost:5455/zeroui_shared" -ForegroundColor White
    Write-Host "  Redis:  redis://localhost:6382" -ForegroundColor White
}

if ($Mode -eq "start" -or ($Mode -eq "setup" -and $StartDockerDeps)) {
    Write-Host "`nShared Services:" -ForegroundColor Cyan
    foreach ($svcName in $DiscoveredServices.Keys) {
        $svc = $DiscoveredServices[$svcName]
        $conn = Get-NetTCPConnection -LocalPort $svc.Port -ErrorAction SilentlyContinue
        if ($conn) {
...
if ($RunUnitTests) {
    Write-Host "`nUnit Tests: Executed - ALL SHARED TESTS PASSED" -ForegroundColor Green
