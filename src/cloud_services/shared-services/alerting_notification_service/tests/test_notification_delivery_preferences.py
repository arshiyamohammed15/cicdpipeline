...
from ..database.models import Alert, Notification, UserNotificationPreference
...
async def test_notification_dispatcher_fallback(session: AsyncSession, tmp_path, monkeypatch):
    ...
    result = await session.exec(select(Notification).where(Notification.notification_id.like("fallback-%")))
