...
class RBACEvaluator:
    ...
    def evaluate(self, roles: List[str], action: str, resource: str) -> Tuple[bool, str]:
        ...
        for role in roles:
            ...
            evaluated_roles.append(canonical_role)
        ...
        if evaluated_roles:
            role_list = ", ".join(sorted(set(evaluated_roles)))
            return False, f"RBAC: {role_list} role(s) lack permission for {action}"
        return False, f"RBAC: No canonical roles available for {action}"
