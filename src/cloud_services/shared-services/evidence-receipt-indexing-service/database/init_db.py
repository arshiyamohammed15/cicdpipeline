"""
Database initialization script for ERIS module.

What: Applies schema.sql to target PostgreSQL database
...
LOGGER = logging.getLogger("eris.init_db")
...
def initialize_schema(database_url: str, force: bool = False) -> None:
    ...
    """
    LOGGER.info("Initializing ERIS database...")
    ...
    try:
        if force:
            ...
            cursor.execute(
                ...
                DROP TABLE IF EXISTS courier_batches CASCADE;
                DROP TABLE IF EXISTS meta_receipts CASCADE;
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Initialize ERIS database schema"
    )
    parser.add_argument(
        "--database-url",
        default=os.environ.get("DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/eris"),
