...
import {
  ...
  GpuInstanceStatus,
  GpuPoolStats,
} from '../../ports/GpuPoolPort';

interface GpuInstanceInternal {
  id: string;
  gpuCount: number;
  gpuType: string;
  memoryGB: number;
...
export class LocalGpuPool implements GpuPoolPort {
  private instances: Map<string, GpuInstanceInternal> = new Map();
  private totalGpus: number;
  private availableGpus: number;
  private gpuTypes: string[];
  private defaultGpuType: string;
  private defaultMemoryGB: number;
  private defaultCompletionDelay: number; // milliseconds

  constructor(
    totalGpus: number = 8,
    gpuTypes: string[] = ['t4', 'v100'],
    defaultGpuType: string = 't4',
    defaultMemoryGB: number = 16,
    defaultCompletionDelay: number = 5000
  ) {
  ...
  async allocate(request: GpuAllocationRequest): Promise<GpuInstance> {
    ...
    const instance: GpuInstanceInternal = {
      id: instanceId,
  ...
  async getStatus(instanceId: string): Promise<GpuInstanceStatus> {
    const instance = this.instances.get(instanceId);
    if (!instance) {
      throw new Error(`GPU instance ${instanceId} not found`);
    }
  ...
  async getPoolStats(): Promise<GpuPoolStats> {
    let allocatedGpus = 0;
    let inUseGpus = 0;
  ...
  private async simulateCompletion(instanceId: string, delay: number): Promise<void> {
