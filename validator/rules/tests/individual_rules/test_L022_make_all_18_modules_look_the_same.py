...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L022")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L022"), ids=lambda tc: tc.test_method_name)
def test_l022_make_all_18_modules_look_the_same(test_case):
    assert test_case.rule_id == "L022"
