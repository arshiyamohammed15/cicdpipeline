...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L026")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L026"), ids=lambda tc: tc.test_method_name)
def test_l026_organize_features_clearly(test_case):
    assert test_case.rule_id == "L026"
