...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L055")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L055"), ids=lambda tc: tc.test_method_name)
def test_l055_build_confidence_not_fear(test_case):
    assert test_case.rule_id == "L055"
