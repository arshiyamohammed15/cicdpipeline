...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L060")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L060"), ids=lambda tc: tc.test_method_name)
def test_l060_automate_wisely(test_case):
    assert test_case.rule_id == "L060"
