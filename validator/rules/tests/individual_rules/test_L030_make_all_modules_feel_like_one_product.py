...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L030")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L030"), ids=lambda tc: tc.test_method_name)
def test_l030_make_all_modules_feel_like_one_product(test_case):
    assert test_case.rule_id == "L030"
