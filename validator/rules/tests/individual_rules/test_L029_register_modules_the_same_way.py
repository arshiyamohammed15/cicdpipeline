...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L029")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L029"), ids=lambda tc: tc.test_method_name)
def test_l029_register_modules_the_same_way(test_case):
    assert test_case.rule_id == "L029"
