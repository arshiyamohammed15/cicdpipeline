...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L023")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L023"), ids=lambda tc: tc.test_method_name)
def test_l023_process_data_locally_first(test_case):
    assert test_case.rule_id == "L023"
