...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L048")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L048"), ids=lambda tc: tc.test_method_name)
def test_l048_build_compliance_into_workflow(test_case):
    assert test_case.rule_id == "L048"
