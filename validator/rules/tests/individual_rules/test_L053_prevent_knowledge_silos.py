...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L053")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L053"), ids=lambda tc: tc.test_method_name)
def test_l053_prevent_knowledge_silos(test_case):
    assert test_case.rule_id == "L053"
