...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L041")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L041"), ids=lambda tc: tc.test_method_name)
def test_l041_success_dashboards_show_business_value(test_case):
    assert test_case.rule_id == "L041"
