...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L036")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L036"), ids=lambda tc: tc.test_method_name)
def test_l036_be_extra_careful_with_private_data(test_case):
    assert test_case.rule_id == "L036"
