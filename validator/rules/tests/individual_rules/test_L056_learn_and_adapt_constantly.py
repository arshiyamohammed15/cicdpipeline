...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L056")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L056"), ids=lambda tc: tc.test_method_name)
def test_l056_learn_and_adapt_constantly(test_case):
    assert test_case.rule_id == "L056"
