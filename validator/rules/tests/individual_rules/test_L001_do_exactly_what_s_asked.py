...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L001")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L001"), ids=lambda tc: tc.test_method_name)
def test_l001_do_exactly_what_s_asked(test_case):
    assert test_case.rule_id == "L001"
