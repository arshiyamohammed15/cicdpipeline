...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L064")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L064"), ids=lambda tc: tc.test_method_name)
def test_l064_be_predictable_and_consistent(test_case):
    assert test_case.rule_id == "L064"
