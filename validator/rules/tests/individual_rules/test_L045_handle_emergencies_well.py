...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L045")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L045"), ids=lambda tc: tc.test_method_name)
def test_l045_handle_emergencies_well(test_case):
    assert test_case.rule_id == "L045"
