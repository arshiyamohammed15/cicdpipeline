...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L003")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L003"), ids=lambda tc: tc.test_method_name)
def test_l003_protect_people_s_privacy(test_case):
