...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L058")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L058"), ids=lambda tc: tc.test_method_name)
def test_l058_catch_issues_early(test_case):
    assert test_case.rule_id == "L058"
