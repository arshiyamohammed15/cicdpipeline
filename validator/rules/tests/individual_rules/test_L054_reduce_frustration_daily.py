...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L054")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L054"), ids=lambda tc: tc.test_method_name)
def test_l054_reduce_frustration_daily(test_case):
    assert test_case.rule_id == "L054"
