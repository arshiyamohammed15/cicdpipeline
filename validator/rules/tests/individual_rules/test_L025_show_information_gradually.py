...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L025")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L025"), ids=lambda tc: tc.test_method_name)
def test_l025_show_information_gradually(test_case):
    assert test_case.rule_id == "L025"
