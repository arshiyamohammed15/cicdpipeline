...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L047")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L047"), ids=lambda tc: tc.test_method_name)
def test_l047_track_problems_you_prevent(test_case):
    assert test_case.rule_id == "L047"
