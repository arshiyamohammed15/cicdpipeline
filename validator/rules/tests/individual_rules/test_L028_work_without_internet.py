...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L028")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L028"), ids=lambda tc: tc.test_method_name)
def test_l028_work_without_internet(test_case):
    assert test_case.rule_id == "L028"
