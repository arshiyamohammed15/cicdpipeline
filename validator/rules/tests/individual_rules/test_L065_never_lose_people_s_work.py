...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L065")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L065"), ids=lambda tc: tc.test_method_name)
def test_l065_never_lose_people_s_work(test_case):
    assert test_case.rule_id == "L065"
