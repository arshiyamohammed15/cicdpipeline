...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L046")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L046"), ids=lambda tc: tc.test_method_name)
def test_l046_make_developers_happier(test_case):
    assert test_case.rule_id == "L046"
