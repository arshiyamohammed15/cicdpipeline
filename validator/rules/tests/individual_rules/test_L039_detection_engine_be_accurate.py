...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L039")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L039"), ids=lambda tc: tc.test_method_name)
def test_l039_detection_engine_be_accurate(test_case):
    assert test_case.rule_id == "L039"
