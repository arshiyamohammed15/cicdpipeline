...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L033")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L033"), ids=lambda tc: tc.test_method_name)
def test_l033_solve_real_developer_problems(test_case):
    assert test_case.rule_id == "L033"
