...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L016")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L016"), ids=lambda tc: tc.test_method_name)
def test_l016_keep_good_logs(test_case):
    assert test_case.rule_id == "L016"
