...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L040")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L040"), ids=lambda tc: tc.test_method_name)
def test_l040_risk_modules_safety_first(test_case):
    assert test_case.rule_id == "L040"
