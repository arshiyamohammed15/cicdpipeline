...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L011")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L011"), ids=lambda tc: tc.test_method_name)
def test_l011_check_your_data(test_case):
    assert test_case.rule_id == "L011"
