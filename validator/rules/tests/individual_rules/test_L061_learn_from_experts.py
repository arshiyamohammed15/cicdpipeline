...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L061")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L061"), ids=lambda tc: tc.test_method_name)
def test_l061_learn_from_experts(test_case):
    assert test_case.rule_id == "L061"
