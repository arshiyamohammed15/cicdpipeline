...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L021")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L021"), ids=lambda tc: tc.test_method_name)
def test_l021_use_the_hybrid_system_design(test_case):
    assert test_case.rule_id == "L021"
