...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L019")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L019"), ids=lambda tc: tc.test_method_name)
def test_l019_keep_different_parts_separate(test_case):
    assert test_case.rule_id == "L019"
