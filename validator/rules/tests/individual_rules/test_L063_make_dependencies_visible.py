...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L063")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L063"), ids=lambda tc: tc.test_method_name)
def test_l063_make_dependencies_visible(test_case):
    assert test_case.rule_id == "L063"
