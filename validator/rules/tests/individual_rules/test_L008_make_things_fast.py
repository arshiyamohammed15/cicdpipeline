...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L008")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L008"), ids=lambda tc: tc.test_method_name)
def test_l008_make_things_fast(test_case):
    assert test_case.rule_id == "L008"
