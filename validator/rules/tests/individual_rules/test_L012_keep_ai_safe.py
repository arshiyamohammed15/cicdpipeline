...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L012")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L012"), ids=lambda tc: tc.test_method_name)
def test_l012_keep_ai_safe(test_case):
    assert test_case.rule_id == "L012"
