...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L018")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L018"), ids=lambda tc: tc.test_method_name)
def test_l018_make_things_repeatable(test_case):
    assert test_case.rule_id == "L018"
