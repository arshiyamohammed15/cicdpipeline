...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L038")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L038"), ids=lambda tc: tc.test_method_name)
def test_l038_mmm_engine_change_behavior(test_case):
    assert test_case.rule_id == "L038"
