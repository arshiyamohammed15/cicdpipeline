...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L002")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L002"), ids=lambda tc: tc.test_method_name)
def test_l002_only_use_information_you_re_given(test_case):
    assert test_case.rule_id == "L002"
