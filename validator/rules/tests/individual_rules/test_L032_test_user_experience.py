...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L032")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L032"), ids=lambda tc: tc.test_method_name)
def test_l032_test_user_experience(test_case):
    assert test_case.rule_id == "L032"
