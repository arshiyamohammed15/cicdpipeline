...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L059")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L059"), ids=lambda tc: tc.test_method_name)
def test_l059_build_safety_into_everything(test_case):
    assert test_case.rule_id == "L059"
