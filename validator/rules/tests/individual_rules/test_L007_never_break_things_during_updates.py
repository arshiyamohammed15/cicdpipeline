...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L007")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L007"), ids=lambda tc: tc.test_method_name)
def test_l007_never_break_things_during_updates(test_case):
    assert test_case.rule_id == "L007"
