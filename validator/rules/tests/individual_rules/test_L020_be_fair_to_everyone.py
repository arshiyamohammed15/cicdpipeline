...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L020")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L020"), ids=lambda tc: tc.test_method_name)
def test_l020_be_fair_to_everyone(test_case):
    assert test_case.rule_id == "L020"
