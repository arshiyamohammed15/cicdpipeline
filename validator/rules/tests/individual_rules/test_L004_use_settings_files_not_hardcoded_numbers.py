...
rule = next(r for r in factory.get_all_rules() if r["id"] == "L004")

@pytest.mark.parametrize("test_case", factory.create_test_cases(lambda x: x.get("id") == "L004"), ids=lambda tc: tc.test_method_name)
def test_l004_use_settings_files_not_hardcoded_numbers(test_case):
    assert test_case.rule_id == "L004"
